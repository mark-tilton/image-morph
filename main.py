...
def warp_point(point):
    yn = point[0]
    xn = point[1]
    xo = math.sin(yn*math.pi*2) * 0.1 # 10% horizontal warp
    yo = math.sin(xn*math.pi) * 0.1 # 10% vertical warp
    return (yn + yo, xn + xo)
...
compensation_grid = np.zeros((grid_w, grid_h, 2))
for yi in range(grid_h):
    for xi in range(grid_w):
        x = xi / (grid_w - 1) * shape[1]
        y = yi / (grid_h - 1) * shape[0]
        grid_point = grid[xi][yi]
        compensation_grid[xi][yi] = (2 * x - grid_point[0], 2 * y - grid_point[1])
...
for y in range(shape[0]):
    ...
    for x in range(shape[1]):
        xn = x / shape[1]
        yi = yn * (compensation_grid.shape[0] - 1)
        xi = xn * (compensation_grid.shape[1] - 1)
        yi0 = math.floor(yi)
        yi1 = yi0 + 1
        xi0 = math.floor(xi)
        xi1 = xi0 + 1
        ynn = yi - yi0
        xnn = xi - xi0
        p00 = vmath.Vector2(compensation_grid[xi0][yi0])
        p01 = vmath.Vector2(compensation_grid[xi0][yi1])
        p10 = vmath.Vector2(compensation_grid[xi1][yi0])
        p11 = vmath.Vector2(compensation_grid[xi1][yi1])
        t = (p10 - p00)
        b = (p11 - p01)
        ...
        ny = int(p[1])
        for c in range(shape[2]):
            sample = warped_img[y, x, c]
            if nx >= 0 and ny >= 0 and nx < shape[1] and ny < shape[0]:
                unwarped_img[ny, nx, c] = sample
